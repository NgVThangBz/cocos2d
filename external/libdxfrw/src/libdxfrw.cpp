        ...
        for (int i = 0;  i< ent->loopsnum; i++){
            ...
            if ( (loop->type & 2) == 2){
                ...
                for (int j = 0; j < loop->numedges; ++j) {
                    if (loop->objlist.at(j)->eType == DRW::LWPOLYLINE) {
                        numbPolyline++;
                    }
                }
                if (numbPolyline > 0) {
                    writer->writeInt16(93, loop->numedges - numbPolyline);
                }
                else {
...
//                        writer->writeInt16(72, 4);
                    ...
                    {
                        if (count == 0) {
                            writer->writeInt16(97, numbPolyline);
                        }
                        writer->writeString(330, toHexStr(entCount + count + 1));
                        count++;
                    }
                        break;
                    default:
...
/*        if (ent->deflines > 0){
        ...
        writer->writeDouble(53, 45.0);
        writer->writeDouble(43, 0.0);
        writer->writeDouble(44, 0.0);
        writer->writeDouble(45, -ent->ltypeScale);
        writer->writeDouble(46, ent->ltypeScale);
        ...
        for (int i = 0; i < ent->loopsnum; i++) {
            ...
            for (int j = 0; j < loop->numedges; ++j) {
                if (loop->objlist.at(j)->eType == DRW::LWPOLYLINE) {
                    DRW_LWPolyline* a = (DRW_LWPolyline*)loop->objlist.at(j);
                    this->writeLWPolyline(a);
                }
            }
        }
